...
def cfg():
    ...
    min_size = 200
    max_slices = 3
    use_gt = True  # True - use ground truth as training label, False - use supervoxel as training label
    eval_fold = 0  # (0-4) for 5-fold cross-validation
    test_label = [1, 4]  # for evaluation, Liver and Spleen
    ...
    # Network
    reload_model_path = (
        "./exps_on_CHAOST2/CATNet_train_CHAOST2_cv2/40/snapshots/100.pth"
    )

    opt = {}
    opt["pretrain_path"] = "./data/resnet_50_23dataset.pth"

    opt["model_depth"] = 50  # Depth of resnet (10 | 18 | 34 | 50 | 101)

    opt["resnet_shortcut"] = "B"  # Shortcut type of resnet (A | B)
